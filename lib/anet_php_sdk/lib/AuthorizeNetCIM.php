...
class AuthorizeNetCIM extends AuthorizeNetRequest
{
    ...
    protected function _setPostString()
    {
        ...
        // Add extraOptions CDATA
        if ($this->_extraOptions) {
        	// Replace extraOptions tag with CDATA'd version, formatted for CIM API specs
            if(strpos($this->_post_string, "<extraOptions></extraOptions>") !== FALSE) {
            	$this->_post_string = str_replace("<extraOptions></extraOptions>",'<extraOptions><![CDATA[' . $this->_extraOptions . ']]></extraOptions>', $this->_post_string);
            }
            else if(strpos($this->_post_string, "<extraOptions/>") !== FALSE) {
            	$this->_post_string = str_replace("<extraOptions/>",'<extraOptions><![CDATA[' . $this->_extraOptions . ']]></extraOptions>', $this->_post_string);
            }
            // Blank out extra options, so that we don't include it in calls that
        ...
        foreach ($array as $key => $value) {
            if ($value && !is_object($value)) {
                if (is_array($value) && count($value)) {
                    ...
                    $destination->addChild($key, htmlspecialchars($value));
                }
