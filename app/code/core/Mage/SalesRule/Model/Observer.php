...
class Mage_SalesRule_Model_Observer
{
    ...
    /**
     * Record usage of rules and associated coupons
     *
     * @param Varien_Event_Observer $observer
     */
    public function sales_order_afterPlace($observer)
    {
        ...
        if (!$order || ($order->getIsMultiShipping() && $order->getSkipUsageCounting())) {
            return $this;
        }
        ...
        // lookup rule ids
        $ruleIds = $quote->getAppliedRuleIds() ? explode(',', $quote->getAppliedRuleIds()) : array();
