...
class Alliance_GiftOfChoice_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function checkGiftOfChoice($checProductInCart = true)
	{
		// Get customer.
		$customer = Mage::helper('customer');
		
		// Check module availability.
		// Mage::log("checkGiftOfChoice.getModuleEnabled: ".$this->getModuleEnabled().".", null, 'giftofchoice.log');
		if(! $this->getModuleEnabled()) return false;
		
		// Check user logged.
		// Mage::log("checkGiftOfChoice.isLoggedIn: ".(Mage::helper('customer')->isLoggedIn() ? 1 : 0).".", null, 'giftofchoice.log');
		$customerGroup = $this->getCustomerGroup();
		if(! $customer->isLoggedIn() && ! in_array(-1, $customerGroup)) return false;
		
		// Check user group.
		// Mage::log("checkGiftOfChoice.userGroup configured: ".print_r($this->getCustomerGroup(), 1)." - ".$customer->getCustomer()->getGroupId().".", null, 'giftofchoice.log');
		if($customer->isLoggedIn() && ! in_array($customer->getCustomer()->getGroupId(), $customerGroup)) return false;
		
		// Check Rule name.
		$ruleApplied = $this->checkRuleName();
		// Mage::log("checkGiftOfChoice.ruleApplied: ".($ruleApplied ? 1 : 0).".", null, 'giftofchoice.log');
		if(! $ruleApplied) return false;
		
		// Check Minimum Order Amount.
		$cartAmount = $this->getApplicableSubtotal();
		$minimumAmount = $this->getMinimumAmount();
		// Mage::log("checkGiftOfChoice: ".$cartAmount." < ".$minimumAmount.".", null, 'giftofchoice.log');
		if($cartAmount == 0 || $cartAmount < $minimumAmount) return false;
		
		// Get product collection.
		$productCollection = $this->getProductCollection();
		// Mage::log("checkGiftOfChoice.productCollection: ".$productCollection->count().".", null, 'giftofchoice.log');
		...
		if($checProductInCart) {
			...
			// Mage::log("checkGiftOfChoice.productsInCart: ".($productsInCart ? 1 : 0).".", null, 'giftofchoice.log');
	...
	public function checkRuleName()
	{
		...
		// Mage::log("checkRuleName: websiteId: ".$websiteId.".", null, 'giftofchoice.log');
		...
		// Mage::log("checkRuleName: ".$ruleIds." - ruleName: ".$ruleName.".", null, 'giftofchoice.log');
		...
		foreach($ruleIdsArray as $_ruleId) {
			...
			// Mage::log("checkRuleName: rule: ".$rule->getName().".", null, 'giftofchoice.log');
	...
	private function _getModuleConfig()
	{
		if(empty($this->_moduleConfig)) {
			$this->_moduleConfig = array(
				'enabled' => Mage::getStoreConfig('alliance_offers/giftofchoice_configuration/enabled'),
				'title' => Mage::getStoreConfig('alliance_offers/giftofchoice_configuration/title'),
				'description' => Mage::getStoreConfig('alliance_offers/giftofchoice_configuration/description'),
				'customer_group' => Mage::getStoreConfig('alliance_offers/giftofchoice_configuration/customer_group'),
				'minimum_amount' => Mage::getStoreConfig('alliance_offers/giftofchoice_configuration/minimum_amount'),
				'category_id' => Mage::getStoreConfig('alliance_offers/giftofchoice_configuration/category_id'),
				'rule_name' => Mage::getStoreConfig('alliance_offers/giftofchoice_configuration/rule_name'),
			);
		}
