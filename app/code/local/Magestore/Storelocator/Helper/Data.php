...
class Magestore_Storelocator_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	{
		$collection = Mage::getModel('adminhtml/system_config_source_country')->toOptionArray();
		
		$allCountriesList = array();
		$storeCountries = array();
		
		foreach ($collection as $item) {
			$allCountriesList[$item['value']] = $item['label'];
		}
		
		$countriesList = Mage::getModel('storelocator/storelocator')
						->getCollection()
						->addFieldToSelect('country')
						->distinct(true);
		
		foreach($countriesList as $c ) {
			$storeCountries[$c->getCountry()] = $allCountriesList[$c->getCountry()];
		}
	...
	public function getStoreCountriesOptions()
	{
		$collection = Mage::getModel('adminhtml/system_config_source_country')->toOptionArray();
		...
		foreach($countriesList as $c ) {
			$storeCountries[] = array('value' => $c->getCountry(), 'label' => $allCountriesList[$c->getCountry()]);
		}
